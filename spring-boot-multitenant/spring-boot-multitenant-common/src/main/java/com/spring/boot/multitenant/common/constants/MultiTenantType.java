...
public enum MultiTenantType{
    /**
     * 租户字段隔离
     * 所有租户共享同一个表,通过表中增加租户ID字段进行区分。
     * 这种方式开发和维护比较简单,但是存在租户数据安全和隔离难保证的问题。
     */
     ...
     * accounts_tenant2 (租户2)
     * 这种方式资源利用最高,迁移方便,但是SQL可能会比较复杂。
     */
    TABLE("TABLE", "共享数据源,同schema,租户ID隔离"),
    /**
     * 共享数据源,不同schema
     * 所有租户共享一个数据源,但每个租户拥有独立的schema。
    ...
    /**
     * 不同数据源
     * 为每个租户创建独立的数据源,使用JPA操作不同的数据源实现隔离。
     * 这种方式实现简单,但是不太实际,因为租户数量较多时,创建和管理大量数据源会很麻烦。
     */
